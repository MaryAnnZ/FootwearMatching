...
\par
In this thesis two ways are considered to increase the quality of a given input, namely, first, to find and enhance the information regardless of the properties and presence of noise, or second, to suppress any kind of noise preserving the information. 
In this chapter an experimental application for the first possibility is introduced. 
%In Chapter 4 an approach for noise suppression is discussed.
%In Chapter 5 a semi-automated enhancement algorithm is presented.
The theoretical background and the structure is discussed in the first section, afterwards implementation details are reported, followed by a qualitative evaluation of the results.
...
\par
Because of the high intra-class variance, a discriminative feature learning algorithm is proposed based on the work of Guo et al. \cite{guo2012discriminative}.
...
In the first layer, where robustness is granted, those features are selected that describe the majority of a given texture.
...
\par
For the purpose of this prototypical implementation only the first two layers are used, because the goal is to distinguish between noise and information, thus two classes, pattern and noise, are defined. 
...
For that reason, features are firstly extracted both from the fore- and background of the samples, where the foreground contains the exact shoeprint pattern and the background is the noisy ground where the shoeprint is lying. 
After this, to achieve high inter-class distance despite the absence of the third layer, frequent descriptors of the noise are determined and all of them are eliminated from the pattern descriptors.
The remaining pattern descriptors are propagated to the next level. 
In case of Fourier-Mellin Transform no such modification is made.
...
Consequently, a similarity measure is used for all three candidate features. If the similarity value reaches a predefined threshold the two descriptors are considered the same, and the operation is applied according to the tasks of the given layer.
At LBP histogram correlation is used for comparison, at SIFT Brute-Force matcher is applied and at the Fourier-Mellin Transform the similarity measure proposed in \cite{gueham2008automatic} used.
