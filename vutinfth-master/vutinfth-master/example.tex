...
Additionally, a line detector is applied and the edges of the shoeprint are clustered based on the differences of the gradients to distinguish between shoeprint impression and noise edges.
...
\begin{figure}[h]
 ...
 \begin{subfigure}[t]{0.13\columnwidth}
  ...
  \end{subfigure}
  \caption{Example images from the FID-300 \cite{kortylewski2014unsupervised} dataset, where the someprint is captures under different conditions.}
...
Firstly, the produced shoeprint and its background highly depends on the properties of the ground where the impression was made, the roughness and unevenness of a given type of surface distorts the original shoe sole pattern and those features on the ground appear as noise on the final sample.
The impressions of Figure \ref{fig:int:varying:1} and \ref{fig:int:varying:20} were made on an even surface whereas on Figure \ref{fig:int:varying:17} and \ref{fig:int:varying:25} cluttered background caused by the surface properties are seen.
...
The securing method and the properties of the floor, for example if it was clean or dusty when making an impression, also determine if the positive \ref{fig:int:cap:pos} or the negative \ref{fig:int:cap:neg}, the pattern or the space between them, image is captured.
...
The used samples are either synthetically generated, they are secured in a laboratory for the purpose of evaluating forensic image processing algorithms, and computationally distorted \cite{de2005automated}, \cite{gueham2008automatic} or exclude low quality and noisy images \cite{dardi2009texture}, \cite{tang2010footwear}.
...
After that, gradient based line detection is performed and the results are separated into clusters, where the clustering criteria is the difference of the gradients in x and y directions.
...
Second, the edges of the shoeprint impressions share similar properties, such as gradient information, frequency as well as overall area and length, and are different from those deriving from cluster of the image.
...
Because of the absence of a standard database, the discussed algorithms are separated into two groups, techniques tested on synthetic samples and on real-life impressions.
...
\par
Histogram Operations are combined apart from Bandpass filtering, like Sugamya et al. \cite{sugamya2016image} propose, but also with Thresholding as suggested by Yao et al. \cite{yao2016image}. 
...
Noise Removal methods are based on estimating the original image and eliminating the deviating features of the data.
...
\par
Shoeprint Description  in varying difficulty and quality of the different datasets is a continuous issue in the research of Shoeprint Identification, therefore the properties of the database the given approach was tested on is highlighted for each case.
