...
Along the literature of image enhancement and noise removal another related topics are also reviewed.
...
Other than that, publications from similar domains such as fingerprint and palmprint detection as well are described.
Research for fingerprint identification have been chosen for review because of their similar goal of edge and image structure recognition.
...
Tables \ref{tab:ref} and \ref{tab:ref:alg} provide an overview about the used datasets of the discussed algorithms.
...
The algorithms presented in the "real" section of the dataset were tested on the FID-300 dataset, also used in this thesis.
...
\par
Morphological Operations are popular techniques for improving the quality of both kind, realistic and synthetic, of input data, namely Opening and Closing, is used in many cases \cite{wang2014automatic}, \cite{kong2014novel}, \cite{li2014retrieval}.
...
Nevertheless, the neglect of thresholding methods for real datasets indicates that they can not defeat the changing conditions on the samples of the FID-300 datset.
...
\hline
Wu et al. (2019) \cite{wu2019crime} & \multirow{6}{*}{ real } & 300 & 93\% @ 2\% \\
Wu et al. (2019) \cite{wu2019losgsr} & & 300 & 68\% @ 1\% \\
Kong et al. (2017) \cite{kong2017cross} & & 300 & 90\% \\
Kong et al. (2019) \cite{kong2019cross} & & 300 & 86\% @ 1\% \\
Kortylewski et al. (2014) \cite{kortylewski2014unsupervised} & & 300 & 86\% @ 20\% * \\
Kortylewski et al. (2016) \cite{kortylewski2016probabilistic} & & 300 & 71\% @ 20\% * \\
\end{tabular}
...
\hline
Wu et al. (2019) \cite{wu2019crime} & \multirow{6}{*}{ real } & Morphological op. , Otsu th. & Gabor Transf., Fourier-Mellin Transf. \\
...
The images are converted to the frequency domain where high and low frequencies are eliminated, Gueham et al. \cite{gueham2007automatic},  Richetelli et al. \cite{richetelli2017classification}.
...
\par
It is outstanding that the majority of reviewed publications working on the FID-300 dataset do not suggest to use enhancement techniques, Table \ref{tab:ref:alg}.
...
Additionally, the presented algorithms in Table \ref{tab:ref}, do not use a-priori knowledge about the samples, no information about the appearance of noise nor of the shoeprint is available.
Because of that, they focus on the definition of robust descriptor and powerful matching technique which finds the corresponding shoeprints, despite the high shoeprint pattern to noise ratio and changing conditions.
...
Similar to forensic shoeprint samples, fingerprint images also depict a 2 dimensional object where the information derives from the line pattern of the sample, Figure \ref{fig:rw:fingerprint}.
The usage of bandpass filtering and signal transform for fingerprint images shows, that the edge content of the input do not influence their performance.
...
This way a filtering method for general noise is built by breaking the down into known parts, into known subbands.  
\par
...
\par
Shoeprint Description in varying datasets is an issue in the research of shoeprint identification, therefore the properties of the database the given approach was tested on is given in Tables \ref{tab:ref} and \ref{tab:ref:alg}.
...
There are other publications along that where Fourier-Mellin Transform is proposed for feature description, e.g. Gueham et al. \cite{gueham2008automatic}.
...
This shows, that Fourier-Mellin features are powerful descriptors for synthetic images, and they are also applicable for lower quality data, such as the samples of FID-300.
\par
...
It is also proposed to combine the feature descriptors, Kong et al. \cite{kong2014novel} incorporate Zernike and Gabor features whereas Tang et al. \cite{tang2010footwear} define their own fundamental shapes based on common basic structures on a shoe sole through Hough Line Transform. 
The extracted features are then stored in an Attributed Relational Graph to represent the entire shoeprint image.
Comparing the datasets the algorithms were introduced for, Table \ref{tav:ref}, it is outstanding that shape descriptors were only introduced for high-quality data and not for samples from FID-300.
On FID-300 images there is a high amount of noise which distorts the original shoeprint pattern.
In this cases shape descriptors are less suitable, because the original outlines of given element are modified or covered by the noise.
\par
...
Similar to the shape descriptors, point descriptors were tested only on high quality data, Table \ref{tab:ref:alg}.
Even though SIFT is a robust descriptor  \cite{lowe1999object}, its neglect by algorithms introduced for FID-300 indicates, that it does not overcome the issue of high signal-to-noise ratio on the samples.
\par
...
Comparing to other descriptors previously discussed in this section, it is outstanding that the majority utilize on machine learning, and introduce a detailed descriptor for low-quality data.
...
In case of shoeprint impressions Fourier-Mellin features were dominating, they were proposed in several publications  \cite{gueham2007automatic}, \cite{wu2019crime} and achieved high accuracy \ref{tab:ref}, but for fingerprint descriptors Gabor Transform and Radon Transform are preferred.
...
Depending on the original shoesole, the elements of the shoeprint are possibly bigger and widespraed on the input image, Figure  \ref{fig:rw:ref}, compared to  fingerprint sample, Figure \ref{fig:rw:fp}.
%\par
...
\par
For natural texture description signal domain features are now reviewed .
...
Descriptors based on signal tranformation are, thus powerful tools, and based on the reviewed literature it is advisable to take advantage on them.
\par
...
As mentioned previously, Ahonen et al. \cite{ahonen2009rotation} use the LBP features of the Fourier domain, similarly, in \cite{guo2010completed} LBPs are calculated twice, after that the input is separated into two components, into signs and magnitudes, to make the basic LBP rotation invariant as well.
...
In their approach a circular neighboring radius and a dominant orientation is stored so that scale invariance is also granted.
...
Even though, LBPs are extensively researched for natural textures, their applicability for forensic shoeprint impressions is questionable.
LBPs were not suggested shoeprint identification by the publications reviewed in the previous sections.
...
This is a possible issue on natural texture images, but on forensic shoeprint impressions, as discussed in the following chapter, distortions and additional noise are the main challenges.
